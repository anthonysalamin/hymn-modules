/**
 * HYMN | initMethodSwitch v1.2
 * @build 19.09.24 @updated 24.03.26
...
export function initMethodSwitch() {
    document.addEventListener("DOMContentLoaded", () => {
      ...
      OPTIONS.SLIDES.forEach((slide) => {
        ...
        // Skip this slide if critical elements are missing
        if (!iconWrap || !iconPlus || !frontWrap) return;
        ...
        if (frontTitle) {
          tl.to(frontTitle, { y: "50%", duration: 0.65, ease: "expo.out" }, 0);
        }
        ...
        if (frontWrap) {
          tl.to(frontWrap, { opacity: 0, duration: 0.65, ease: "expo.out" }, "<");
        }
